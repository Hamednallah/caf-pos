...
public class OrdersDAOTest {
    ...
    @Test
    public void createOrderTransactional() throws Exception {
        ItemsDAO itemsDAO = new ItemsDAO(conn);
        com.yourcompany.cafeteria.model.Item item = new com.yourcompany.cafeteria.model.Item();
        item.setName("OItem");
        item.setPrice(new BigDecimal("5.00"));
        int itemId = itemsDAO.insert(item);
        ...
        oi.setItemId(itemId);
        oi.setQuantity(1);
        oi.setLineTotal(new BigDecimal("5.00"));
        o.setItems(List.of(oi));

        OrdersDAO dao = new OrdersDAO(conn);
        int orderId = dao.createOrderTransactional(o);
        assertTrue(orderId > 0);

        OrderItemsDAO oidao = new OrderItemsDAO(conn);
        var items = oidao.listByOrderId(orderId);
